...
public class ControladorVuelo implements ActionListener {
	...
	public void crear() {
	    ...
	    try {
	        ...
	        if(modelo.crear(nuevo)) {
	            ...
	            JOptionPane.showMessageDialog(null, "ERROR: No se pudo adicionar el Vuelo (posiblemente duplicado).", "Error", JOptionPane.ERROR_MESSAGE);
	        }
	        
	    } catch (NumberFormatException e) {
	         JOptionPane.showMessageDialog(null, "Error: Verifique que el número de vuelo, filas y columnas sean números.", "Error de Entrada", JOptionPane.ERROR_MESSAGE);
	    } catch (Exception e) {
	        JOptionPane.showMessageDialog(null, "Ocurrió un error inesperado al crear el vuelo.", "Error", JOptionPane.ERROR_MESSAGE);
	    }
	}
	...
	public void leer () {
		    ...
		    try {
		        ...
		        JOptionPane.showMessageDialog(null, "Ingrese un número de vuelo válido.", "Error de Entrada", JOptionPane.ERROR_MESSAGE);
		        return;
		    }
		    ...
		    if(vueloE == null) {
		        ...
		        if (asientos != null) { 
		            ...
		            DefaultTableModel Asientost = (DefaultTableModel) this.vista.tablaAsientos.getModel();
		            ...
		            JOptionPane.showMessageDialog(null, "El vuelo no tiene asientos asignados.", "Advertencia", JOptionPane.WARNING_MESSAGE);
		        }
		    } 
		}
		public void actualizar() {
			...
			if(respuesta == JOptionPane.YES_OPTION) {
				...
				Date fechaHoraSalida = (Date) this.vista.spinner.getValue();
				vuelo.setFechaHoraSalida(fechaHoraSalida);
				
				Vuelo vueloN = new Vuelo();
				modelo.actualizar(index, vuelo);
				JOptionPane.showMessageDialog(null, "Los datos se mofificaron");
		...
		public void eliminar() {
		    ...
		    try {
		        ...
		        JOptionPane.showMessageDialog(null, "Ingrese un número de vuelo válido para eliminar.", "Error", JOptionPane.ERROR_MESSAGE);
		        return;
		    }
		    ...
		    Vuelo vueloE = (Vuelo) modelo.leer(buscar); 
		    		    ...
		    		    if (vueloE == null) {
		        JOptionPane.showMessageDialog(null, "El vuelo con el número " + buscar.getNumero() + " no fue encontrado.", "Advertencia", JOptionPane.WARNING_MESSAGE);
		        return; 
		    }
		    
		    
		    Asiento[][] asientosDelVuelo = vueloE.getAsiento(); 
		    int indexAEliminar = modelo.buscarIndex(vueloE); 
		    int contador = 0;
		    ...
		    if (asientosDelVuelo != null) {
		        ...
		        for (int i = 0; i < asientosDelVuelo.length; i++){
		            ...
		            for ( int j = 0; j < asientosDelVuelo[i].length ; j++){ 
		                ...
		                if (asientosDelVuelo[i][j] != null && asientosDelVuelo[i][j].getPasajero() != null) {
		                    contador++;
		                }
		            }
		        }
		    ...
		    if (contador == 0) {
		        ...
		        if (respuesta == JOptionPane.YES_OPTION) {
		            boolean eliminado = modelo.eliminar(indexAEliminar); 
		            
		            if (eliminado) {
		                JOptionPane.showMessageDialog(null, "El vuelo ha sido eliminado exitosamente.");
		            
		                this.limpiarCampos(); 
		            } else {
		                JOptionPane.showMessageDialog(null, "Error: No se pudo eliminar el vuelo.", "Error", JOptionPane.ERROR_MESSAGE);
		            }
		        }
		    } else {
		...
		private void limpiarCampos() {
		    ...
		    DefaultTableModel asientosModel = (DefaultTableModel) this.vista.tablaAsientos.getModel();
		    asientosModel.setRowCount(0); 
			    ...
			    public void leerTodos() {
			        ...
			        DefaultTableModel tableModel = (DefaultTableModel) this.vista.table.getModel();
			        ...
			        if (listaVuelos.isEmpty()) {
			            JOptionPane.showMessageDialog(null, "No hay vuelos registrados en el sistema.", "Información", JOptionPane.INFORMATION_MESSAGE);
			        }
			    }
